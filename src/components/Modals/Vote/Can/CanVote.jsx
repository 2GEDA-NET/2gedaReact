...
const CanVote = ({ voteOptions, creator, question, duration }) => {
  ...
  return (
    <div className="can-vote">
      ...
      <div className="vote">
        ...
        <div className="poll">
          {voteOptions && voteOptions.length > 0 && voteOptions.map((item, index) => (
             <div className="lylac" onClick={() => handleVote(index)}>
             <p style={{ fontSize: "12px", textAlign: "start" }}>{item.content}</p>
             <div
               className={`egg-bg ${getHighestIndex() === index ? "highest" : ""}`}
             >
